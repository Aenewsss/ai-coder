...
export class DeepSeekProvider implements LLMProvider {
  ...
  constructor(apiKey: string, model = 'deepseek-chat', maxTokens = 8192) {
  ...
  private convertMessages(
    ...
    return messages.map((msg) => {
      ...
      if (msg.role === 'assistant') {
        ...
        // Include reasoning_content if present (required for DeepSeek thinking mode)
        if (msg.reasoning_content) {
          assistantMsg.reasoning_content = msg.reasoning_content;
        }

        return assistantMsg as OpenAI.Chat.ChatCompletionAssistantMessageParam;
      }
